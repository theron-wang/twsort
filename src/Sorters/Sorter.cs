...
{
    private ProjectConfigurationManager ProjectConfigurationManager { get; } = ProjectConfigurationManager.Instance;
    ...
    {
        var projectCompletionValues = ProjectConfigurationManager.GetCompletionConfigurationByFilePath(filePath).Result;

        var classOrder = ClassSortUtilities.GetClassOrder(projectCompletionValues.Version);
        var variantOrder = ClassSortUtilities.GetVariantOrder(projectCompletionValues.Version);
        ...
        var result = classes
            ...
            .ThenBy(className =>
            {
                ...
                if (classToSearch.Contains('-'))
                {
                    ...
                    if (projectCompletionValues.CustomSpacing.TryGetValue($"{stem}-{{0}}", out var spacing))
                    {
                        if (spacing.Contains(ending))
                        {
                    ...
                    else if (projectCompletionValues.Version == TailwindVersion.V3 && projectCompletionValues.Spacing.Contains(ending) ||
                        (
                            ending.Length > 2 &&
                            ending[0] == '[' &&
                    ...
                    else if (projectCompletionValues.CustomColors.TryGetValue($"{stem}-{{0}}", out var colors))
                    {
                        if (colors.Contains(ending))
                        {
                    ...
                    else if (projectCompletionValues.Colors.Contains(ending.Split('/')[0]) ||
                            (
                                ending.Length > 2 &&
                                ending[0] == '[' &&
                                ending[ending.Length - 1] == ']' &&
                    ...
                    else
                    {
                        if (stem.Contains('-'))
                        {
                            ...
                            var colorStem = classToSearch[..splitIndex];
                            var colorEnding = classToSearch[(splitIndex + 1)..];

                            if (colorEnding.Contains('/'))
                            {
                                colorEnding = colorEnding[..colorEnding.IndexOf('/')];
                            }

                            if (projectCompletionValues.CustomColors.TryGetValue($"{colorStem}-{{0}}", out colors))
                            {
                                if (colors.Contains(colorEnding))
                                {
                            ...
                            else if (projectCompletionValues.Colors.Contains(colorEnding))
                            {
                                classToSearch = $"{colorStem}-{{c}}";
                            }
                            else
                    ...
                    if (shouldKeepLooking)
                    {
                        if (projectCompletionValues.Version >= TailwindVersion.V4)
                        {
                            ...
                            {
                                classToSearch = $"{classToSearch[..classToSearch.IndexOfAny(['[', '('])].Trim('-')}-{{a}}";
                            }
                        }
